...
- **⚠️ 修改流程（铁律）**：任何代码改动前，先讨论细节 → 不确定的地方先问用户 → 用户确认后再动手 → 分步进行，每步完成后等用户确认再继续下一步。禁止不经确认直接改代码。
- **⚠️ 推送铁律**：禁止主动 `git push` 或 `git commit`。只有用户明确说"上传"、"推送"、"commit"、"push"时才执行。构建验证 `npm run build` 可以自己做。
- **⚠️ 新建功能必问**：每次新建任何功能、新建数据库表时，必须问用户「这个功能的数据需不需要支持导入/导出？」，用户确认后如果需要，就把该数据加入 `src/utils/exportData.ts` 和 `src/utils/importData.ts`。
- 每阶段完成后暂停并确认
- 用通俗语言解释技术决策
- 尊重用户的审美偏好（简洁温暖）
- 优先保证核心功能稳定，再考虑花哨特性
